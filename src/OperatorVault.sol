...
import "core/base/Vault.sol";
import "core/interfaces/IOperatorVCS.sol";
import "core/interfaces/IOperatorStaking.sol";
import "core/interfaces/IPFAlertsController.sol";
...
contract OperatorVault is Vault {
    ...
    function raiseAlert(address _feed) external onlyOperator {
        ...
        uint256 opRewards = (rewards * IOperatorVCS(vaultController).operatorRewardPercentage()) / 10_000;
    ...
    function getPendingRewards() public view returns (uint256) {
        ...
        if (depositChange > 0) {
            return (uint256(depositChange) * IOperatorVCS(vaultController).operatorRewardPercentage()) / 10_000;
        }
        ...
        returns (uint256, uint256)
    {
        ...
        uint256 opRewards;
        if (depositChange > 0) {
            opRewards = (uint256(depositChange) * IOperatorVCS(vaultController).operatorRewardPercentage()) / 10_000;
