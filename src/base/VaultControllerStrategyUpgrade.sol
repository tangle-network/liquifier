...
abstract contract VaultControllerStrategyUpgrade is Strategy {
    ...
    function getPendingFees() external view override returns (uint256) {
        ...
        if (balanceChange > 0) {
            for (uint256 i = 0; i < fees.length; i++) {
                totalFees += (uint256(balanceChange) * fees[i].basisPoints) / 10_000;
            }
        }
        return totalFees;
    }
     ...
     */
    function migrateVaults(uint256 _startIndex, uint256 _numVaults, bytes calldata _data) external onlyOwner {
