...
import "core/interfaces/IStaking.sol";
...
abstract contract VaultControllerStrategy is Strategy {
        ...
        onlyInitializing
    {
        ...
        if (_maxDepositSizeBP > 10_000) revert InvalidBasisPoints();
            ...
            + (
                stakeController.isActive()
                    ? MathUpgradeable.min(
                        ...
                        ((stakeController.getMaxPoolSize() - stakeController.getTotalPrincipal()) * maxDepositSizeBP) / 10_000
                    )
                    : 0
            );
    }
     ...
     */
    function upgradeVaults(uint256 _startIndex, uint256 _numVaults, bytes memory _data) external onlyOwner {
