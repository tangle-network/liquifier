...
import "core/base/VaultControllerStrategy.sol";
import "core/interfaces/IOperatorVault.sol";
...
contract OperatorVCS is VaultControllerStrategy {
        ...
        reinitializer(3)
    {
        if (address(token) == address(0)) {
            __VaultControllerStrategy_init(_token, _stakingPool, _stakeController, _vaultImplementation, _fees, _maxDepositSizeBP);

            if (_operatorRewardPercentage > 10_000) revert InvalidPercentage();
    ...
    function getPendingFees() external view override returns (uint256) {
        ...
        if (depositChange > 0) {
            for (uint256 i = 0; i < fees.length; ++i) {
                totalFees += (uint256(depositChange) * fees[i].basisPoints) / 10_000;
            }
        }
        return totalFees;
    }
    ...
    function getMaxDeposits() public view override returns (uint256) {
        if (preRelease) {
            return stakeController.isActive() ? 855_000 ether : 0;
        }
        ...
        if (depositChange > 0) {
            ...
            if (receivers.length == 0) {
                ...
                for (uint256 i = 0; i < receivers.length; ++i) {
                    ...
                    amounts[i] = (uint256(depositChange) * fees[i].basisPoints) / 10_000;
                }
            } else {
                for (uint256 i = 1; i < receivers.length; ++i) {
                    receivers[i] = fees[i - 1].receiver;
                    amounts[i] = (uint256(depositChange) * fees[i - 1].basisPoints) / 10_000;
                }
            }
